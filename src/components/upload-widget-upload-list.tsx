import { useUpload } from "../store/uploads";
import * as ScrollArea from "@radix-ui/react-scroll-area";
import { UploadWidgetUploadItem } from "./upload-widget-upload-item";

export function UploadWidgetUploadList() {
    const uploads = useUpload(state => state.uploads);
    ...
    return (
        <div className="px-3 flex flex-col gap-3">
            <span className="text-zinc-400 text-sm">
                Uploaded files{' '}
                <span className="text-zinc-500 text-xs">({uploads.size})</span>
            </span>
            <ScrollArea.Root type="scroll" className="overflow-hidden">
                <ScrollArea.Viewport className="h-[200px]">
                    {
                        isUploadListEmpty ? (
                            <span className="text-zinc-400 text-xs">No upload added.</span>
                        ) : (
